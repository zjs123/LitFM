...
def _generate_summary_prompt(data_point: dict, eos_token: str, instruct: bool = False):
    ...
    Q = Q + "Each citation should be indicated with \cite."
    Q = Q + human_instruction[1] + "\n"
    return Q
...
def test_sentence(model_name):
    ...
    for i in range(100): #range(len(test_data)):
        ...
        t_nei = list(nx.all_neighbors(raw_graph, target)) # node_id_2_retrieval_papers[target] #
    ...
    pickle.dump(result_dict, open("sentence_example_phy_vicuna.pkl", 'wb'))
...
def test_LP(model_name):
    ...
    for i in range(500): #range(len(test_data)):
        ...
        if 'GPT' in model_name:
            prompt = _generate_LP_prompt(datapoint, '') #_generate_LP_prompt_neighbor(datapoint, tokenizer.eos_token)
            ans = gpt_35_api([{'role': 'user','content': prompt}])
        else:
            prompt = _generate_LP_prompt(datapoint, tokenizer.eos_token) #_generate_LP_prompt_neighbor(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
    ...
    for i in range(500): #range(len(test_data)):
        ...
        s_nei = list(nx.all_neighbors(raw_graph, source))
        s_nei_list = list(set(s_nei) - set([source]) - set([target]))[:1]
        s_nei_titles = [raw_id_2_tile_abs[i][0] for i in s_nei_list]
        ...
        try:
            ...
            t_nei = []
        t_nei_list = list(set(t_nei) - set([source]) - set([target]))[:1]
        t_nei_titles = [raw_id_2_tile_abs[i][0] for i in t_nei_list]
...
def test_retrival_e(model_name):
    result_list = []
    # pos test
    for i in range(200): #range(len(test_data)):
        source, target = test_data[i][0], test_data[i][1]
        source_title, source_abs = raw_id_2_tile_abs[source]
        target_title, target_abs = raw_id_2_tile_abs[target]
        ...
        if 'GPT' in model_name:
            ...
            prompt, label = _generate_retrival_prompt(datapoint, tokenizer.eos_token) #_generate_retrival_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_retrival_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
        res = ans.strip().split(human_instruction[1])[-1]
...
def test_title_generate(model_name):
    ...
    for i in range(100): #range(len(test_data)):
        ...
        retrieval_nei = list(nx.all_neighbors(raw_graph, source)) #node_id_2_retrieval_papers[source] #list(nx.all_neighbors(raw_graph, source)) #node_id_2_retrieval_papers[source]
    ...
    pickle.dump(result_dict, open("title_example_phy_vicuna.pkl", 'wb'))
...
def test_abs_completion(model_name):
    ...
    for i in range(100): #range(len(test_data)):
        ...
        if 'GPT' in model_name:
            ...
            ans = gpt_35_api([{'role': 'user','content': prompt}])
        if 'vicuna' in model_name:
            prompt = _generate_abstrat_completion_prompt(datapoint, tokenizer.eos_token) #_generate_abstrat_2_title_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_abstrat_2_title_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
        if 'lora' in model_name:
            prompt = _generate_abstrat_completion_prompt(datapoint, tokenizer.eos_token) #_generate_abstrat_2_title_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_abstrat_2_title_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
    ...
    pickle.dump(result_dict, open("abs_example_phy_vicuna.pkl", 'wb'))
...
def test_summary(model_name):
    ...
    # pos test
    for i in list(related_work_data.keys())[:1000]:
        source = i
        related_work_ground_truth = related_work_data[i][0]
        if len(related_work_ground_truth) == 0:
            continue
    ...
    pickle.dump(result_dict, open("summary_example_phy_vicuna13b.pkl", 'wb'))
...
if __name__ == "__main__":
    ...
    if args.model == 'lora': 
        ...
        adapter_save_path = f"{config['model_output_dir']}/{config['model_name']}_adapter_physics_title_abs" #_physics_title_abs  physics_LP_retrieval_sentence
    ...
    graph_data = nx.read_gexf("physics_final_processed_with_attributes.gexf", node_type=None, relabel=False, version='1.2draft')
    test_data = pickle.load(open('test_physics.pkl', 'rb'))
    raw_graph = graph_data
    related_work_data = pickle.load(open('related_work_ground_truth_arxiv_physics.pkl', 'rb'))
    print([len(graph_data.nodes()), len(graph_data.edges())])

    raw_id_2_tile_abs = dict()
    for node in list(graph_data.nodes()):
        title = graph_data.nodes()[node]['Title']
        abstract = graph_data.nodes()[node]['Abstract']
        raw_id_2_tile_abs[node] = [title, abstract]
