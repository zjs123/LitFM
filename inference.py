...
def _generate_LP_prompt(data_point: dict, eos_token: str, instruct: bool = False):
    ...
    Q = Q + "Determine if paper A will cite paper B. Give me a direct answer of yes or no"
...
def _generate_abstrat_completion_prompt_neighbor(data_point: dict, eos_token: str, instruct: bool = False):
    ...
    Q = Q + "Here are abstracts pf other papers related to paper A: \n"
...
def _generate_summary_prompt(data_point: dict, eos_token: str, instruct: bool = False):
    ...
    Q = Q + "Title of the Paper A : " + data_point['s_title'] + "\n"
    Q = Q + "Abstract of the Paper A : " + data_point['s_abs'] + "\n"
    Q = Q + "Here are some papers cited by Paper A and their citation sentences: \n"
    ...
    Q = Q + "Please organize these citation sentences as the related work section of paper A. \n" # vicuna7b without lora
    #Q = Q + "Organize these citation sentences as a related work section. \n" # vicuna7b without lora
    Q = Q + human_instruction[1] + "\n"
...
def _generate_summary_cluster_prompt(data_point: dict, eos_token: str, instruct: bool = False):
    ...
    #Q = Q + "Please organize these citation sentences as a paragraph. \n"
    #Q = Q + "Please organize these citation sentences as a paragraph which can be used as a related work section. \n"
    Q = Q + "Give me the related work section of paper A."
    Q = Q + "You should first group these papers, and then summary each group, finally organize them as a paragraph. \n" # vicuna7b without lora
    #Q  = Q + 'For each cited paper, you should indicate it with a number, e.g., [1].'
    #Q = Q + "Organize these citation sentences as a related work section. \n" # vicuna7b without lora
    Q = Q + human_instruction[1] + "\n"
...
def test_sentence(model_name):
    ...
    for i in range(len(test_data)):
        ...
        t_nei = random.sample(list(raw_graph.nodes()), 10)#list(nx.all_neighbors(raw_graph, target)) #node_id_2_retrieval_papers[target] #list(nx.all_neighbors(raw_graph, target))
        ...
        for i in range(len(t_nei_list)):
            tmp_sentence = raw_id_pair_2_sentence[(t_nei_list[i], target)][0][0] if (t_nei_list[i], target) in raw_id_pair_2_sentence.keys() else ''
        ...
        if 'GPT' in model_name:
            prompt = _generate_sentence_prompt(datapoint, '') #_generate_sentence_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_sentence_prompt(datapoint, tokenizer.eos_token)
...
def test_LP():
    ...
    for i in range(500): #range(len(test_data)):
        ...
        s_nei = random.sample(list(raw_graph.nodes()), 10)#+list(nx.all_neighbors(raw_graph, source)) #node_id_2_retrieval_papers[source] #list(nx.all_neighbors(raw_graph, source))
        ...
        prompt = _generate_LP_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_LP_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_LP_prompt(datapoint, tokenizer.eos_token)
        ans = get_llm_response(prompt)[0]['generated_text']
     ...
     # neg test
    for i in range(500): # range(len(test_data)):
        source, target = test_data[i][0], (test_data[i][1]+200)%len(raw_id_2_tile_abs.keys())
        source_title, source_abs = raw_id_2_tile_abs[source]
        target_title, target_abs = raw_id_2_tile_abs[target]

        s_nei = random.sample(list(raw_graph.nodes()), 10)#node_id_2_retrieval_papers[source] #list(nx.all_neighbors(raw_graph, source))
        s_nei_list = list(set(s_nei) - set([source]) - set([target]))[:10]
        s_nei_titles = [raw_id_2_tile_abs[i][0] for i in s_nei_list]

        try:
            t_nei = random.sample(list(raw_graph.nodes()), 10)#node_id_2_retrieval_papers[target] #list(nx.all_neighbors(raw_graph, target))
        except:
            t_nei = []
        t_nei_list = list(set(t_nei) - set([source]) - set([target]))[:10]
        t_nei_titles = [raw_id_2_tile_abs[i][0] for i in t_nei_list]
...
def test_retrival_e(model_name):
    ...
    for i in range(len(test_data)):
        ...
        retrieval_nei = random.sample(list(raw_graph.nodes()), 10) #node_id_2_retrieval_papers[source] # neighbors
        retrieval_nei_list = list(set(retrieval_nei) - set([source]) - set([target]))[:10]
        ...
        if 'GPT' in model_name:
            prompt, label = _generate_retrival_prompt(datapoint, '') #_generate_retrival_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_retrival_prompt(datapoint, tokenizer.eos_token)
            ans = gpt_35_api([{'role': 'user','content': prompt}])
        else:
            prompt, label = _generate_retrival_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_retrival_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_retrival_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
        res = ans.strip().split(human_instruction[1])[-1]
        ...
        print(target_title)
        print(res)
        if target_title[:int(len(target_title)*0.8)] in res or res in target_title or 'Paper'+str(label) in res:
            result_list.append(1)
        else:
            result_list.append(0)
        print([sum(result_list), len(result_list)])
...
def test_title_generate(model_name):
    ...
    for i in range(100): #range(len(test_data)):
        ...
        if 'GPT' in model_name:
            prompt = _generate_abstrat_2_title_prompt(datapoint, '') #_generate_abstrat_2_title_prompt_neighbor(datapoint, tokenizer.eos_token) #_generate_abstrat_2_title_prompt(datapoint, tokenizer.eos_token)
            ans = gpt_35_api([{'role': 'user','content': prompt}])
        else:
        ...
        result_dict[abstract] = [title, res]
    ...
    #pickle.dump(result_dict, open("title_generate_result.pkl", 'wb'))
...
def test_abs_completion(model_name):
    ...
    for i in range(100): #range(len(test_data)):
        ...
        retrieval_nei = node_id_2_retrieval_papers[source]
        retrieval_nei_list = list(set(retrieval_nei) - set([source]) - set([target]))[:5]
        ...
        if 'GPT' in model_name:
            prompt = _generate_abstrat_completion_prompt(datapoint, '') #_generate_abstrat_completion_prompt(datapoint, tokenizer.eos_token)
            ans = gpt_35_api([{'role': 'user','content': prompt}])
        else:
            prompt = _generate_abstrat_completion_prompt(datapoint, tokenizer.eos_token) #_generate_abstrat_completion_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
        res = ans.strip().split(human_instruction[1]+'\n')[-1]

        result_dict[abstract] = [abstract, res]
    ...
    pickle.dump(result_dict, open("abs_comp_result.pkl", 'wb'))
...
def test_summary():
    ...
    for i in range(200):
        ...
        source_title, _ = raw_id_2_tile_abs[source]
        
        s_nei = list(nx.all_neighbors(raw_graph, source))
        s_nei_list = list(set(s_nei) - set([source]))#[:10]
        s_nei_titles = [raw_id_2_tile_abs[i][0] for i in s_nei_list]
        s_nei_sentences = []
        for sampled_nei in s_nei_list:
            citation_sentence = raw_id_pair_2_sentence[(source, sampled_nei)][0][0] if (source, sampled_nei) in raw_id_pair_2_sentence.keys() else raw_id_pair_2_sentence[(sampled_nei, source)][0][0]
            s_nei_sentences.append(citation_sentence)
        if len(s_nei_sentences) >= 10:
            datapoint = {'s_title':source_title, 's_abs':source_abs, 'nei_title':s_nei_titles, 'nei_sentence': s_nei_sentences}
            prompt = _generate_summary_cluster_prompt(datapoint, tokenizer.eos_token)
            ans = get_llm_response(prompt)[0]['generated_text']
            res = ans.strip().split(human_instruction[1]+'\n')[-1]

            result_dict[source_title] = [source_abs, s_nei_titles, s_nei_sentences, res]

            print(prompt)
            print(res)
    #pickle.dump(result_dict, open("summary_result_min10_cluster_lora.pkl", 'wb'))
...
if __name__ == "__main__":
    ...
    if args.model == 'lora': 
        ...
        adapter_save_path = f"{config['model_output_dir']}/{config['model_name']}_adapter_LP_retrieval_sentence" # LP_retrieval_sentence
    ...
    title_2_abs = pickle.load(open("title_2_abs.pkl","rb"))
    retrieval_results = pickle.load(open("retrieval_result_all_test_nodes_pubmed.pkl","rb"))
    node_id_2_retrieval_papers = {}
    for i in range(len(retrieval_results[0])):
        query = retrieval_results[0][i]
        pred = retrieval_results[1][i]
        node_id_2_retrieval_papers[int(query)] = pred
    #print(node_id_2_retrieval_papers.keys())
    ...
    for i in range(len(edge_list)):
        source, target = edge_list[i][0], edge_list[i][1]
        source_title, source_abs = raw_id_2_tile_abs[source]
        target_title, target_abs = raw_id_2_tile_abs[target]
        pos = {'s_title':source_title, 's_abs':source_abs, 't_title':target_title, 't_abs':target_abs, 'label':'yes'}

        source, target = edge_list[i][0], (edge_list[i][1]+215)%len(raw_id_2_tile_abs.keys())
        source_title, source_abs = raw_id_2_tile_abs[source]
        target_title, target_abs = raw_id_2_tile_abs[target]
        neg = {'s_title':source_title, 's_abs':source_abs, 't_title':target_title, 't_abs':target_abs, 'label':'no'}

        instruction += _generate_LP_prompt(pos, tokenizer.eos_token) + pos['label'] + '\n' + _generate_LP_prompt(neg, tokenizer.eos_token) + neg['label'] + '\n'   
    '''
    
    
    #test_LP(args.model)
    #test_retrival_e(args.model)
    #test_title_generate(args.model)
    #test_sentence(args.model)
    test_abs_completion(args.model)
    #test_summary()

# python inference.py -config_path=configs/vicuna_7b_qlora_uncensored.yaml -model=vicuna
